...
def demo():
    ...
    print("=" * 70)
    print("DIGITAL TWIN OF TRANSFORMER - DEMONSTRATION")
    print("=" * 70)
    
    for hour in range(3):
        # Simulate varying load and temperature
        base_current = 400 + 50 * np.sin(2 * np.pi * hour / 24)
        oil_temp = 65 + 10 * np.sin(2 * np.pi * hour / 24)
        
        reading = SensorReading(
            timestamp=datetime.utcnow().isoformat(),
            voltage_primary_kv=11.0 - 0.2 * np.random.random(),
            current_primary_a=base_current,
            voltage_secondary_kv=0.4,
            current_secondary_a=base_current * 360,
        ...
        oil_quality = OilQuality(
            moisture_ppm=250 + 50 * np.sin(2 * np.pi * hour / 72),
        ...
        if hour % 1 == 0:
            ...
            print(f"\n[{report['timestamp']}]")
            print(f"  Transformer ID: {report['transformer_id']}")
            print(f"  Overall Health: {report['overall_health']}")
            print(f"\n  Performance Metrics:")
            print(f"    Load: {report['summary']['load_percent']:.1f}%")
            print(f"    Efficiency: {report['summary']['efficiency']:.2f}%")
            print(f"    Oil Temp: {report['summary']['oil_temperature']:.1f}°C")
            print(f"    Hot Spot: {report['summary']['hot_spot_temperature']:.1f}°C")
            print(f"\n  Condition Metrics:")
            print(f"    Health Index: {report['summary']['health_index']:.1f}/100")
            print(f"    Remaining Life: {report['summary']['remaining_life_years']:.1f} years")
            
            if report['active_faults']:
                print(f"\n  ⚠️  Active Faults:")
                for fault in report['active_faults']:
                    print(f"    - {fault['type']} ({fault['severity']})")
                    threshold = fault.get('threshold', 'N/A')
                    print(f"      Value: {fault['value']:.2f} | Threshold: {threshold}")
...
if __name__ == "__main__":
    demo()
    print("\n✅ Digital Twin demonstration completed!")
    print("\nNext steps:")
    print("  1. Replace demo data with real sensor data")
    print("  2. Calibrate models using historical records")
    print("  3. Deploy with real-time MQTT/Kafka data stream")
    print("  4. Build monitoring dashboard")
    print("  5. Integrate with CMMS system")
"""